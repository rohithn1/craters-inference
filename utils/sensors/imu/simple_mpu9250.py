...
def talker():
    ...
    while not rospy.is_shutdown():
		...
		print(
			f"Angular Velocity - x: {imu.GyroVals[0]:.3f}, y: {imu.GyroVals[1]:.3f}, z: {imu.GyroVals[2]:.3f}"
		)
		print(
			f"Linear Acceleration - x: {imu.AccelVals[0]:.3f}, y: {imu.AccelVals[1]:.3f}, z: {imu.AccelVals[2]:.3f}"
		)